...
public class AgencyListFragment extends BaseFragment implements
		OnClickListener, OnRefreshListener<ListView> {
	/**
	 * 友盟统计
	 */
	protected boolean isCreated = false;
	private static final int AGENCY=1;//适配那边判断 0就是学校 1是机构
	public static String TAG = "SchoolListFragment";
	private MyAcademyAdapter mAdapter;
	private List<DxAgencyinfo> mList = new ArrayList<DxAgencyinfo>();
	private List<DxAgencyinfo> mListFlush = new ArrayList<DxAgencyinfo>();
	...
	public AgencyListFragment(EtutorApplication application, Activity activity,
			Context context) {
	...
	public View onCreateView(LayoutInflater inflater,
		...
		// TODO Auto-generated method stub
		mView = inflater.inflate(R.layout.fragment_agency_list, container,
				false);
	...
	protected void initViews() {
		...
		mListView = (PullToRefreshListView) findViewById(R.id.lv_agency);

		mMap = new HashMap<String, Object>();
		mMap.put("start", start);
		mMap.put("pageSize", pageSize);
		mAdapter = new MyAcademyAdapter(mList,this.getActivity(),AGENCY);
	 ...
	 *            0:刷新 1:加载更多
	 */
	private void getAgencyList(final int type) {
		// TODO Auto-generated method stub
		...
		String urlString = UrlEngine.searchAgencyinfo(mMap);
					...
					try {
						...
						if (response.length() != 0) {
							...
							for (int i = 0; i < response.length(); i++) {
								DxAgencyinfo dxAgencyinfo = new DxAgencyinfo();
								dxAgencyinfo
										.initWithAttributes((JSONObject) response
												.get(i));
								mListFlush.add(dxAgencyinfo);
							}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == Activity.RESULT_OK) {
			...
			case 101:
				...
				mMap.put("classify", data.getStringExtra("classify"));
				mMap.put("start", start);
				mMap.put("pageSize", pageSize);
				getAgencyList(0);
				break;
