...
public class WheelView extends View {
	...
	/** Text size */
	private static final int TEXT_SIZE = mScreenHeight / 35;

	/** 顶部和底部项目偏移（隐藏） */
	private static final int ITEM_OFFSET = TEXT_SIZE / 10;

	/** 项目布局的附加宽度 */
	private static final int ADDITIONAL_ITEMS_SPACE = mScreenHeight / 100;

	/** Label offset */
	private static final int LABEL_OFFSET = 8;

	/** 左、右填充值 */
	private static final int PADDING = mScreenHeight / 50;
	...
	public void setAdapter(WheelAdapter adapter, int type) {
		this.adapter = adapter;
		initCurrentItem(type);
		invalidateLayouts();
		invalidate();
	}

	private void initCurrentItem(int type) {
		// TODO Auto-generated method stub
		switch (type) {
		case 1:
			String province = EtutorApplication.getInstance().getSpUtil()
					.getProvince();
			if (TextUtils.isEmpty(province)) {
				province = EtutorApplication.getInstance().getSpUtil()
						.getLocationCity();
			}
			...
			break;
		case 2:
			String city = EtutorApplication.getInstance().getSpUtil().getCity();
			for (int i = 0; i < adapter.getItemsCount(); i++) {
				if (!TextUtils.isEmpty(city)) {
					if (adapter.getItem(i).equals(city)) {
						currentItem = i;
					}
				} else {
					currentItem = 0;
			...
			break;
		case 3:
			String region = EtutorApplication.getInstance().getSpUtil().getRegion();
			for (int i = 0; i < adapter.getItemsCount(); i++) {
				if (!TextUtils.isEmpty(region)) {
					if (adapter.getItem(i).equals(region)) {
						currentItem = i;
					}
				} else {
					currentItem = 0;
			...
			break;
		case 4:
			String category = EtutorApplication.getInstance().getSpUtil().getCategory();
			for (int i = 0; i < adapter.getItemsCount(); i++) {
			...
			break;
		case 5:
			String subject = EtutorApplication.getInstance().getSpUtil().getSubject();
			for (int i = 0; i < adapter.getItemsCount(); i++) {
				if (!TextUtils.isEmpty(subject)) {
					if (adapter.getItem(i).equals(subject)) {
						currentItem = i;
					}
				} else {
					currentItem = 0;
			...
			break;
		case 6:
			String classity = EtutorApplication.getInstance().getSpUtil().getClassify();
			for (int i = 0; i < adapter.getItemsCount(); i++) {
				if (!TextUtils.isEmpty(classity)) {
					if (adapter.getItem(i).equals(classity)) {
						currentItem = i;
					}
				} else {
					currentItem = 0;
